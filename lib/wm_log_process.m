...
%cope with the case that the blue side was not being checked
do_output_pd_check=isfield(data.wm_log.raw,'read_all_adc');
if do_output_pd_check
    data.wm_log.raw.read_all_adc.posix_time = sort(data.wm_log.raw.read_all_adc.posix_time);
end

do_blue_check=do_output_pd_check;
...
for ii=1:iimax
    ...
    time_lower=abs_anal_opts.trig_dld_on_main_comp+anal_opts.probe.t0-anal_opts.wm_log.global.fall_time-0.5; %the probe turns on
    time_upper=abs_anal_opts.trig_dld_on_main_comp+anal_opts.probe.t0-anal_opts.wm_log.global.fall_time+anal_opts.probe.duration+0.5; %when the probe beam goes off
