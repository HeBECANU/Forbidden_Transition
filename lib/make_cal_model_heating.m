function out=make_cal_model_heating(anal_opts_cal,data)
...
cal_dat_mask=col_vec(data.mcp_tdc.all_ok) & temp_cal;
...
probe_dat_mask=col_vec(data.mcp_tdc.all_ok) & ~temp_cal;
...
signal_cal=col_vec(data.signal.heating.cal.val(:,1));
...
calibrated_signal.val=data.signal.heating.msr.val(:,1)*nan; %initalize
calibrated_signal.unc=data.signal.heating.msr.val(:,1)*nan; %initalize
temp_mask = data.mcp_tdc.probe.calibration;
temp_mask(isnan(temp_mask))=0;
temp_mask = ~temp_mask;
temp_msr_mask =~isnan(data.mcp_tdc.probe.calibration(logical(temp_mask)));
calibrated_signal.val(temp_msr_mask)=(data.signal.heating.msr.val(temp_msr_mask,1)-out.signal_cal_drift_model(time_probe));%./data.ai_log.pd.integrated(probe_dat_mask);
